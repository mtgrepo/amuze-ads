...
export class AdvertiserPostsService {
  ...
  async create(createAdvertiserPostDto: CreateAdvertiserPostDto, photoUrl: string): Promise<AdvertiserPost> {
    try {
  ...
  async findOne(id: string): Promise<AdvertiserPost> {
    try {
      const data = await this.advertiserPostRepository.findOne({ where: { id }, relations: ['advertiser'] });
      if(!data) {
